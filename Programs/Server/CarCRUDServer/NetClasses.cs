    ...
    namespace Networking
    {
        ...
        public class NetClient
        {
            ...
            public void SetAddressReusability(bool _reusable)
            {
                ...
                //socket?.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseUnicastPort, _reusable);
                //socket?.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, _reusable);
            }
            ...
            public async void PingRemoteAsync(int _timeInSec, string _ip = null)
            {
                string ip = ValidateIPAddresses(_ip);
                if (ip == null)
                    return;

                await PingRemote(_timeInSec, ip);
            }

            private async Task<bool> PingRemote(int _timeInSec, string _ip = null)
            {
                float tick = _timeInSec;
                while (tick >= 0)
                {
                    byte[] data = Encoding.UTF8.GetBytes("Praise the Sun!");
                    SendUDPAsync(data, _ip);
            ...
            #region UDP
            public async void SendUDPAsync(byte[] _data, string _ip = null)
            {
                string ip = ValidateIPAddresses(_ip);
                if (ip == null) return;

                await SendUDP(_data, ip);
            }

            private async Task<bool> SendUDP(byte[] _data, string _ip)
            {
                socket.SendTo(_data, new IPEndPoint(IPAddress.Parse(_ip), port));
                return true;
            }

            public async void ReceiveUDPAsync(string _ip = null)
            {
                string ip = ValidateIPAddresses(_ip);
                if (ip == null || !receiveEnabled) return;

                try { socket.Bind(new IPEndPoint(IPAddress.Parse(ip), port)); }
                catch { return; }

                await ReceiveUDP(ip);
            }

            private async Task<bool> ReceiveUDP(string _ip)
            {
                IPEndPoint remote = new IPEndPoint(IPAddress.Parse(_ip), port);
                EndPoint ep = remote;

                while (receiveEnabled)
                {
                    byte[] data = new byte[66000];
                    int receivedBytes = await Task.Run(() => socket.ReceiveFrom(data, ref ep));

                    byte[] result = new byte[receivedBytes];
                    Array.Copy(data, result, receivedBytes);

                    remote = ep as IPEndPoint;
                    if (remote.Address.ToString() == _ip && OnMessageReceivedUDPEvent != null)
                        OnMessageReceivedUDPEvent.Invoke(this, ref result);
                }
                return true;
            }
            #endregion
